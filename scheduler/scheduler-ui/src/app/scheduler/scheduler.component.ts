...
// timer subscriptions to get the jobs
  ...
  specialJobs: { jobName: string, status: boolean, time: Date } [] = [];
  ...
  getJobs() {
    {
      this._schedulerService.getJobs().subscribe(
        ...
        err => {
          this._matSnackBar.open("Error while getting all jobs", 'Okay', {
            duration: 1000,
      ...
      success => {
        if (success.statusCode == ServerResponseCode.SUCCESS) {
          ...
          this._dialog.open(MessageComponent, {
            data: "Job with same name exists, Please choose different name.",
            width: '500px'
          })
...
// This will be used when ever the radio buttons are toggled
      ...
      success => {
        if (success.statusCode == ServerResponseCode.SUCCESS && success.data == true) {
          ...
          if (success.statusCode == ServerResponseCode.JOB_NOT_IN_PAUSED_STATE) {
            this._dialog.open(MessageComponent, {
              data: "Job is not in paused state, so cannot be resumed.",
              width: '500px'
            })
      ...
      success => {
        if (success.statusCode == ServerResponseCode.SUCCESS && success.data == true) {
          ...
          if (success.statusCode == ServerResponseCode.JOB_ALREADY_IN_RUNNING_STATE) {
            this._dialog.open(MessageComponent, {
              data: "Job already started/completed, so cannot be paused.",
              width: '500px'
            })
            this.getJobs();
          }
    ...
    this._schedulerService.deleteJob(data).subscribe(
      success => {
        if (success.statusCode == ServerResponseCode.SUCCESS && success.data == true) {
          ...
          if (success.statusCode == ServerResponseCode.JOB_ALREADY_IN_RUNNING_STATE) {
            this._dialog.open(MessageComponent, {
              data: "Job is already started/completed, so cannot be deleted.",
              width: '500px'
            })
  ...
  /***When ever the job name changes the value will be assigned to the jobName and the jobs initial value will be set to '' */
  assignJob(value: String) {
    this.schedulerForm.patchValue({jobName: value})
    for (let a of this.availableJobs) {
      if (a.job_name==value && a.cron_enabled == false) {
  ...
  getSpecialJobs() {
    this._schedulerService.getSpecialJobs().subscribe(data => {
      this.specialJobs = data.data;
      console.log(this.specialJobs);

    })
  }


  onClick(jobName: string) {
    this._schedulerService.stopSpecialJob(jobName).subscribe(value => {
      this.getSpecialJobs();
    })
