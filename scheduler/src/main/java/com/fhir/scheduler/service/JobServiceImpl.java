...
import com.fhir.scheduler.util.*;
import org.quartz.*;
...
//                        interrupted.deleteIfExists(jobName);
                    ...
                    map.put("Stop", checkStop(jobName, 0));

                    map.put("isCronEnabled", checkStop(jobName, 1));
...
//					System.out.println("Job Name:"+jobName + ", Group Name:"+ groupName + ", Schedule Time:"+scheduleTime);
    ...
    private boolean checkStop(String jobName, int var) {
...
//               System.out.println(job.);
    ...
    public boolean addHttpJob(HttpJobRequest req) {

        int exists = jobs_repo.findByJob_name(req.getJobName());
        if (exists > 0) {
            return false;
        } else {
            Available_jobs job = new Available_jobs();
            job.setJob_name(req.getJobName().trim().toUpperCase());
            job.setStart_url(req.getStarturl());
            if (req.getStopurl() == "") {
                job.setStop_url(null);
            } else {
                job.setStop_url(req.getStopurl());
            }
            job.setCron_enabled(req.isEnableCron());
            job.setStatus(false);
            jobs_repo.save(job);
            return true;
        }
    ...
    @Override
    public boolean addClassJob(ClassJobRequest req) {
        int exists = jobs_repo.findByJob_name(req.getJobName());
        if (exists > 0) {
            return false;
        } else {
            Available_jobs job = new Available_jobs();
            job.setJob_name(req.getJobName().trim().toUpperCase());
            job.setClass_path(req.getClasspath().trim());
            job.setStart_method(req.getStartmethod().trim());
            if (req.getStopmethod() == "") {
                job.setStop_method(null);
            } else {
                job.setStop_method(req.getStartmethod().trim());
            }
    ...
    @Override
    public boolean updateHttpJob(UpdateHttpReq req) {

        if (req.getJobName() != null) {
            try {
                Available_jobs job = jobs_repo.getOne(req.getJobName());
                job.setParameters(null);
                job.setStop_method(null);
                ...
                job.setStart_url(req.getStartUrl().trim());
                job.setCron_enabled(req.isEnableCron());
    ...
    @Override
    public boolean updateClassJob(UpdateClassJob req) {
        ...
        if (req.getJobName() != null) {
            try {
                ...
                if (req.getStopMethod() == "") {
                    ...
                    job.setStop_method(req.getStopMethod().trim());
                }
                job.setStart_method(req.getJobName().trim());
                job.setClass_path(req.getClassPath().trim());
                job.setStop_url(null);
    ...
    @Override
    public boolean stopSpecialJob(String jobName) {
        if (jobName != null) {
            Available_jobs jobs = jobs_repo.findAvailable_jobsByJob_name(jobName);
            try {


                RestTemplate req = new RestTemplate();

                ResponseEntity<String> entity = req.getForEntity(jobs.getStop_url(), String.class);

                if (entity.getStatusCodeValue() == 200) {
                    History history = new History(jobName, JobResponseCode.INTERRUPT_FAILED, JobResponseCode.INVALID_STOP_URL, null, new Date());

                }

            } catch (Exception e) {
                History history = new History(jobName, JobResponseCode.INTERRUPT_FAILED, JobResponseCode.INVALID_STOP_URL, null, null);
            } finally {
                jobs.setStatus(false);
                jobs_repo.save(jobs);
            }
        }
    ...
    @Override
    public List<Map<String, Object>> getSpecialJobs() {
       List<Available_jobs> jobs = jobs_repo.getJobs();
       List<Map<String,Object>> list = new ArrayList<>();
       for(Available_jobs a : jobs){
           HashMap<String,Object> map = new HashMap<>();
           map.put("jobName",a.getJob_name());
           map.put("status",a.getStatus());
           map.put("time",a.getStart_time_sp());
           list.add(map);
       }
       return list;
